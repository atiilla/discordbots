...
client.on("message", async message => {
  if (message.content.startsWith("!ping")) {
    message.channel.send(`Webs0cket latency is ${client.ws.ping}ms`)
  }
});

client.on('message', async (message) => {
  if (message.content.startsWith("hello") || message.content.startsWith("hi") || message.content.startsWith("merhaba")) {
...
client.on("guildMemberAdd", async member => {
  ...
  let WELCOME = new Discord.MessageEmbed()
    ...
    .setDescription(`Welcome To Our Community ${member.user}! you are member number ${member.guild.memberCount}! #happyhacking.`)
    .setColor('RED')
...
client.login(process.env.WELCOMER_TOKEN); // Login with the token make sure to add it when starting
